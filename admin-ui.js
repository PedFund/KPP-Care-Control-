    ...
    return `
      <div class="admin-user-card" data-user-id="${user.id}">
        ...
        <div class="admin-stats-grid">
          ...
          <!-- Текущая неделя -->
          <div class="admin-stat-block">
            <h4>Текущая неделя (${week.daysWithData}/${week.totalDays} дней)</h4>
            <div class="admin-stat-row">
              <span>Средние шаги:</span>
              <span>${week.avgSteps.toLocaleString('ru-RU')}</span>
            </div>
            <div class="admin-stat-row">
              <span>Зарядка:</span>
              <span>${week.morningExercise.done}/${week.morningExercise.total} (${week.morningExercise.percentage}%)</span>
            </div>
            <div class="admin-stat-row">
              <span>Тренировки:</span>
              <span>${week.workout.done}/${week.workout.total} (${week.workout.percentage}%)</span>
            </div>
            <div class="admin-stat-row">
              <span>Пресс:</span>
              <span>${week.abs.done}/${week.abs.total} (${week.abs.percentage}%)</span>
            </div>
          </div>
        </div>
      </div>
