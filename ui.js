...
// ОБНОВЛЕНО: 2026-01-09 - Исправлен рендеринг питания с распределением по категориям
      ...
      <div class="history-section">
        ...
        <div class="history-grid">
          ${weeks.map(week => {
            ...
            const className = dominantValue === 0 ? 'success' : 
...
// === ИЗМЕРЕНИЯ (ИСПРАВЛЕНО) ===
// Обновлено: 2026-01-09 - Добавлен показ целевого веса и прогресса
...
async function renderMeasurements() {
  ...
  // Получаем целевой вес из userData
  const targetWeight = currentUserData.targetWeight || null;
  ...
  if (currentWeight && targetWeight) {
    ...
    const progressText = diff > 0 
      ...
      : diff < 0 
        ? `Превышен целевой вес на: <strong>${diffAbs.toFixed(1)} кг</strong>`
        : `<strong>Цель достигнута! 🎉</strong>`;
    
    progressBlock = `
      <div class="measurements-progress" style="background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); color: white; padding: 15px; border-radius: 8px; margin-bottom: 20px;">
        <div style="display: flex; justify-content: space-around; text-align: center;">
          ...
          <div>
            ...
            <div style="font-size: 1.8em; font-weight: bold;">${targetWeight.toFixed(1)} кг</div>
          </div>
        </div>
    ...
    progressBlock = `
      <div class="measurements-progress" style="background: #f0f0f0; padding: 15px; border-radius: 8px; margin-bottom: 20px;">
        <p style="text-align: center; margin: 0;">
          Целевой вес: <strong>${targetWeight.toFixed(1)} кг</strong><br>
          <span style="color: #666; font-size: 0.9em;">Добавьте первое измерение веса</span>
        </p>
      </div>
    `;
  }
          ...
          <tbody>
            ${measurementsList.slice(0, 20).map(m => `
              <tr>
                ...
                <td style="max-width: 200px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap;">${m.comment || '—'}</td>
              </tr>
            `).join('')}
          </tbody>
        </table>
