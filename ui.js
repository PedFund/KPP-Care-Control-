          ...
          <tbody>
            ${measurementsList.slice(0, 30).map(m => {
              const toGoal = m.weight && targetWeight 
                ? (m.weight - parseFloat(targetWeight)).toFixed(1) 
                : '—';
              const toGoalClass = toGoal !== '—' && parseFloat(toGoal) > 0 ? 'style="color: #e74c3c;"' : 
                                  toGoal !== '—' && parseFloat(toGoal) < 0 ? 'style="color: #27ae60;"' : '';
...
function prepareChartData(users) {
  // Получаем все уникальные даты (последние 30 дней)
  const today = getDateKey();
  const dates = [];
  for (let i = 29; i >= 0; i--) {
  ...
  users.forEach((user, index) => {
    ...
    // Данные по датам для каждой метрики
    const nutrition = dates.map(date => user.history[date]?.nutrition ?? null);
    const steps = dates.map(date => user.history[date]?.totalSteps ?? null);
    const morning = dates.map(date => user.history[date]?.morningExercise === 1 ? 1 : 0);
    const workouts = dates.map(date => user.history[date]?.workout === 1 ? 1 : 0);
    const abs = dates.map(date => user.history[date]?.abs === 1 ? 1 : 0);
    const water = dates.map(date => user.history[date]?.water ?? null);
    ...
    datasets.nutrition.push({
      ...
      data: nutrition,
      borderColor: color,
      backgroundColor: color + '30', // прозрачность 30%
      borderWidth: 2,
      tension: 0.3,
      fill: false,
      spanGaps: true
