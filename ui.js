    ...
    options: {
      ...
      scales: {
        ...
        y: {
          ...
          // Для да-нет графиков: показываем только 0 и 1
          min: ['morning', 'workouts', 'abs'].includes(metricKey) ? 0 : metricKey === 'water' ? 0 : undefined,
          max: ['morning', 'workouts', 'abs'].includes(metricKey) ? 1 : metricKey === 'water' ? 6 : undefined,
          // Для да-нет: padding чтобы не обрезалось
          grace: ['morning', 'workouts', 'abs'].includes(metricKey) ? '10%' : undefined,
          ticks: {
            stepSize: ['morning', 'workouts', 'abs'].includes(metricKey) ? 1 : metricKey === 'water' ? 1 : undefined,
            // Для воды: показывать только целые значения 0-6
            autoSkip: metricKey === 'water' ? false : true,
